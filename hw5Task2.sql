...
SET t1.time_to_next_station = COALESCE(TIMESTAMPDIFF(MINUTE, t1.station_time, t2.next_station_time), 0); -- Если следующей станции нет, то мы присваиваем значение 0 столбцу "time_to_next_station" с помощью функции COALESCE().

SELECT trains_id, station,  station_time,
 SEC_TO_TIME(time_to_next_station * 60) AS time_to_next_station -- выбираем все строки из таблицы "train_station" и конвертируем значения столбца "time_to_next_station" в формат времени с помощью функции SEC_TO_TIME().
FROM train_station;
